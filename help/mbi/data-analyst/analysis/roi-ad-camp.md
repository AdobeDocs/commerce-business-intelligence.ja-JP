...
* 広告コストデータを読み込みます。
  * [お客様の [!DNL Google AdWords] から [!DNL Commerce Intelligence]](../importing-data/integrations/google-adwords.md)への接続：これにより、[!DNL Commerce Intelligence]での[!DNL Adwords]の支出が同期されます
...
1. ドロップダウンを使用して、`date`列が順序付けした[!DNL Adwords...] テーブル（変更）の`adCost`列で&#x200B;**合計**&#x200B;を実行します。
...
1. [!UICONTROL ROI]:「\[`A`\]」が`Ad Cost by Campaigns`、「\[`B`\]」が`New users by campaigns`、「\[`C`\] `LTV by campaigns`」の場合、数式`(\[C\]-\[A\]/\[B\])/(\[A\]/\[B\])`を入力します。 これは、（平均ユーザーLTV – 獲得単価）/（獲得単価）の比率を返します
...
開始する前に、[&#x200B; サポートチケット &#x200B;](https://experienceleague.adobe.com/ja/docs/commerce-knowledge-base/kb/troubleshooting/miscellaneous/mbi-service-policies)を送信して、次のディメンションを行項目テーブル （`sales\_flat\_order\_item, order\_item`）に結合します。
...
1. [!UICONTROL Time period]を`All-time`に、[!UICONTROL Interval]を`None`に設定します
1. 「`Filter by`」タブで、`product name 'IN'`製品`A`、製品`B`、製品`C`、...」を追加します。 キャンペーンがターゲットとするすべての製品名をコンマで区切って含めます（例：`product name 'IN' yellow t-shirt`、`red t-shirt, blue t-shirt`）
...
1. 次に、[!DNL AdWords Cost]指標を追加し、上記の`User acquisition campaigns` セクションで検索した`Ad cost by campaigns` レポートのフィルターと方向でグループ化します。次に、指標のスカラー値の下にある&#x200B;**[!UICONTROL Hide]**&#x200B;をクリックします
1. これらの指標を使用して、式を追加します。
1. [!UICONTROL ROI]: `\[A\]`が`Revenue per campaign for specific product(s)`を表し、`\[B\]`が`Ad cost by campaigns`を表す場合、数式`\[A\]/\[B\]`を入力します。 （特定の製品の収益） / （キャンペーンコスト）の比率を返します
