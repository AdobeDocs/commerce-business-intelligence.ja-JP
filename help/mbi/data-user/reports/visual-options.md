...
名前が示すように `table` レポートは、表形式の詳細を表示するのに最適です。 1 つのレポートに、値や指標で大量のグループを表示する必要がある場合は、テーブルが最適な方法です。 例えば、以下に「顧客の詳細」のテーブルを示し、顧客の E メールでグループ化された注文と売上高を示します。
...
* レポート設定の要件はありませんが、テーブルの行数は 3500 行に制限されていることに注意する必要があります。 データセットに 3500 行を超える場合、結果をフィルターして範囲を絞り込むか、結果をに書き出す必要があります。 `.csv` または `Excel` をクリックして、完全なデータセットを表示します。
...
個別の指標、グループ化、時間間隔の組み合わせが、それぞれ独自のバーとして表示されます。 1 つの指標を持つ 2 つの指標がある場合 `group by`(3 つの異なる `group by` の値を指定すると、レポートに 6 つの異なるバーが表示されます。
...
例えば、以下のレポートには 2 つの同じ売上高指標があります。1 つは初回の注文に対してフィルターし、もう 1 つは繰り返し注文に対してフィルターします。 ストアでグループ化した後は、各ストアの合計売上高貢献度（バーの合計幅で表されます）と、初回の場合と各ストアの売上高分類の繰り返しの場合の両方を確認できます。
...
`Column` グラフは、各データポイントを垂直列で表し、通常、横棒グラフビジュアライゼーションよりも時間トレンドデータを表示する方が適しています。 一意の指標とグループを組み合わせて表すのは、それぞれ独自の棒の列なので、通常、列レポートは、3 つ以下の指標を含むレポートでは最適です。1 ～ 3 の値を含む 1 つのグループを持つ 1 つの指標です。

次の例では、2 つの売上高指標を示しています。1 つは初回売上高用に、もう 1 つは繰り返し売上高用にフィルターされ、もう 1 つは月別のトレンドを示しています。
...
ファネルレポートでは、ファネルの特定のステップの相対値がステップの高さに反映され、ステップの表示順はレポート設定によって決まります。 ファネルレポートを設定する方法は 2 つあります。
...
A `bubble` グラフには、最大 4 次元のデータを表示できます。 `X` および `Y` 軸は、吹き出しの位置を指定します。 `Z` [ 軸 ] は気泡のサイズです。2 つのグループを含めると、気泡に色を追加できます。 このタイプのビジュアライゼーションは、複数のデータのディメンションを 1 つのグラフにプロットする場合に最も適しています。
