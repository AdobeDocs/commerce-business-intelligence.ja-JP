...
の機能の 1 つ [!DNL Visual Report Builder] はグローバル `Time Range` および `Interval` 設定。 これらの設定を使用すると、特定の期間のレポート内のデータを分析できます。
...
一部の指標では、データが収集されず、関連するタイムスタンプと共に保存されないので、経時的なトレンドを示すことはできません。 例えば、在庫テーブルには、多くの場合、各 SKU に対して 1 つの行のみが含まれます。 その場合は、 [指標の作成](../data-user/reports/ess-manage-data-metrics.md) タイムスタンプを指定しないでください。
...
1. を追加します。 `Revenue` 指標をレポートに追加します。
1. クリック **[!UICONTROL Duplicate]** をクリックして、指標のコピーを作成します。
1. グローバル **[!UICONTROL Time Range]** オプションを選択し、 **[!UICONTROL Moving Time Range]**. 次に設定： `Last Year`.
1. グローバル **[!UICONTROL Time Interval]** 」オプションを選択し、 `Monthly`.
1. Report Builderは、2 番目の指標の 2 番目の Y 軸を自動的に追加します。 選択を解除すると、 `Multiple Y-Axes` ボックス。
...
1. クリック **[!UICONTROL Time Range]**&#x200B;を、 **[!UICONTROL Moving Time Range]**. 次に設定： `Last Month`.
...
1. ドロップダウンで、以下を設定します。
   ...
   * 次に、カスタム `Day number of the month` クリックによるディメンション **[!UICONTROL Group By]** をクリックし、ディメンションを選択します。 注文月の日数が返されます。例えば、3 月 2 日に発注された注文は次の日に返されます `2`.

   * Adobe Analytics の `Group By` ドロップダウン、選択 `Show All` をクリックします。 **[!UICONTROL Apply]**. これにより、レポートの X 軸値が作成されます。
...
1. を追加します。 `Items Sold` 指標をレポートに追加します。
1. クリック **[!UICONTROL Duplicate]** をクリックして、指標のコピーを作成します。
...
1. 次の日： `Items sold last 7 days` 指標を使用する場合は、グローバル **[!UICONTROL Time Range]** オプションを選択した場合 **[!UICONTROL Moving Time Range]**. この例では、をに設定します。 `Last 7 Days`.
1. クリック **[!UICONTROL Time Interval]** を設定し、 `None`.
...
1. ドロップダウンで、以下を設定します。
   ...
   * クリック **[!UICONTROL Apply]** をクリックして、間隔と範囲の設定を保存します。
   * を複製します。 `Items sold last 28 days` 指標を選択し、新しい指標の `Time Options`. オプションを次のように設定します。

      * `Time Interval`：これはのままにします。 `None`.
      * `Time Range`：をクリックして、目的のプロモーションに適合する日付範囲に変更します **[!UICONTROL Specific Date Range]** そして適切な日付を入力します。
      * 指標の名前を変更 `Items sold during last promotion` 似たようなものです。
      * を追加します。 `Units on hand` 指標。
...
* の `last 7 days time period`，と入力します。 `D / A` （内） `Formula` フィールドに入力します。
* の `last 28 days time period`，と入力します。 `D / (B/4)` （内） `Formula` フィールドに入力します。

  >[!NOTE]
  >
  >ここで選択した期間を正規化することが重要です。 この例では、28 日を 4 週間に分割します。 場合によっては、数式に別のロジックを適用する必要があります。

* の `last promo period`，と入力します。 `D / C` （内） `Formula` フィールドに入力します。
