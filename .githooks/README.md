...
- **SVG**: アイコンおよびシンプルなグラフィックに使用（プリコミットフックによって自動最適化されません）
...
プリコミットフックは、以下を自動的に処理します。

- **PNG** （`.png`） – 可逆および非可逆圧縮
- **JPEG** （`.jpg`、`.jpeg`） – メタデータのクリーンアップによる非可逆圧縮
- **GIF** （`.gif`） – アニメーションと静的な最適化

**メモ**:SVGの最適化はデフォルトで無効になっています（複雑なベクターグラフィックやアニメーションを壊す可能性があります）。 SVG ファイルは、プリコミットフックによって自動的に処理されません。
